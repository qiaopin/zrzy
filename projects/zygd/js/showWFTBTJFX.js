...
function StatisticTBbyCity() {
    ...
    var quart = top.quart;
    parent.layer.loadIndex = parent.layer.load(0, {shade: 0.1});
    JCTBStatisticsManagerService("/StatisticTBbyCity2", {nd: year, jd: quart,xzqdm:sessionStorage.userXZQDM}, true, function (resjson) {
        if (resjson.indexOf("false") > -1) {
            parent.layer.alert("未查询到结果!");
            parent.layer.close(parent.layer.loadIndex);
            var resjosn = [];
...
function StatisticTBbyCounty(smc) {
    ...
    var quart = top.quart;
    parent.layer.loadIndex = parent.layer.load(0, {shade: 0.1});
    JCTBStatisticsManagerService("/StatisticTBbyCounty2", {smc: smc, nd: year, jd: quart,xzqdm:sessionStorage.userXZQDM}, true, function (resjson) {
        if (resjson.indexOf("false") > -1) {
            parent.layer.alert("未查询到结果!");
            parent.layer.close(parent.layer.loadIndex);
            var resjosn = [];
